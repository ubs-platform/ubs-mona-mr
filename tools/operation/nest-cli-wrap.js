...
class NestJsCliWrap {
    ...
    async extendLib(libPath) {
        ...
        const libraryFullPath = path_1.default.join(this.workingDirectory, libPath);
        const libName = path_1.default.basename(libraryFullPath);
        const fullLibName = path_1.default.join(configs.nestCliJson['defaultLibraryPrefix'], libName);
        const tsPublishJsonPath = path_1.default.join(libraryFullPath, 'tsconfig.lib-publish.json');
        const libPackageJsonPath = path_1.default.join(libraryFullPath, 'package.json');
        await FileSystem.copyFile(path_1.default.join(libraryFullPath, 'tsconfig.lib.json'), tsPublishJsonPath);
        const tsPublishObj = await json_util_1.JsonUtil.readJson(tsPublishJsonPath);
        if (tsPublishObj['excludes'] == null) {
            tsPublishObj['excludes'] = [];
        }
        tsPublishObj['excludes'].push(path_1.default.join(configs.nestCliJson['defaultLibraryPrefix'], '**'));
